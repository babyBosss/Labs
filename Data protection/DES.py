...
# в котором один ключ используется как для шифрования, так и для расшифрования данных.
# Процесс шифрования состоит в начальной перестановке, 16 циклах шифрования и конечной перестановке.
# DES имеет блоки по 64 бит и 16 цикловую структуру сети Фейстеля,
# для шифрования использует ключ с длиной 56 бит (в преобразовании участвуют раундовые ключи по 48 бит).
...
class MyDES:
    ...
    def _replace_block(block: str, replace_table: tuple) -> str:
        """
                 Замена отдельного блока
        Args:    block: str, 64-битная строка 01 для преобразования
    ...
    def _processing_encode_input(self, enter: str) -> list:
         # Преобразует входную строку в двоичную форму и разделяет ее на группы по 64 бит
        result = []
        ...
        # Исходный текст преобразован в двоичный
        return result
    ...
    def _key_conversion(self, key: str):
        # Преобразует 64-битный исходный ключ в 56-битный ключ и выполняет перестановку
    ...
    def _key_selection_replacement(self, key: str):
        # 48-битный подключ, выбрав перестановку
        self.child_keys = []
        key_select_table = (
    ...
    def _init_replace_block(self, block: str):
        # Выполняет замену начального состояния на блоке
        replace_table = (
    ...
    def _s_box_compression(self, num: int, block48: str) -> str:
        ...
        Args: num: первая итерация
              block48: right
        Return: 32-битная строку 01, сжатая блоком S
        """
        result_not_or = self._not_or(block48, seаf.child_keys[num])
    ...
    def _p_box_replacement(self, block32: str) -> str:
        """
        Замена P
        Return: 32-битная строка 01 после замены блока P
        """
        p_box_replace_table = (
...
if __name__ == '__main__':
    key = str(input("Введите ключ шифрования: "))
    md = MyDES()
    word = str(input("Введите строку для щифрования: "))
    des_word = md.encode(word, key)
    print("Зашифрованные данные DES:" + des_word)
    print("Расшифрованные данные:" + md.decode(des_word, key))
